...
class ImageSelection:
    ...
    def convert_preview_to_original_coords(self, preview_x, preview_y):
        ...
        # Remove the canvas offset (where image is positioned)
        relative_x = preview_x - image_x
        relative_y = preview_y - image_y
        
        # For zoom out (scale < 1), we need to scale up to get original coordinates
        # For zoom in (scale > 1), we need to scale down to get original coordinates
        if width_scale < 1:
    ...
    def cut_image(self):
        try:
            ...
            if orig_end_x <= orig_start_x or orig_end_y <= orig_start_y:
